...
public class JobRemotingUtil {
    ...
    public static final String BORTUS_JOB_ACCESS_TOKEN = "BORTUS-JOB-ACCESS-TOKEN";
    ...
    public static ReturnT postBody(String url, String accessToken, int timeout, Object requestObj,
        ...
        try {
            ...
            if (accessToken != null && accessToken.trim().length() > 0) {
                connection.setRequestProperty(BORTUS_JOB_ACCESS_TOKEN, accessToken);
            }
            ...
            if (statusCode != 200) {
                return new ReturnT<String>(ReturnT.FAIL_CODE,
                        "rpc remoting fail, StatusCode(" + statusCode + ") invalid. for url : " + url);
            }
            ...
            try {
                ...
                return new ReturnT<String>(ReturnT.FAIL_CODE,
                        "rpc remoting (url=" + url + ") response content invalid(" + resultJson + ").");
            }

        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return new ReturnT<String>(ReturnT.FAIL_CODE,
                    "rpc remoting error(" + e.getMessage() + "), for url : " + url);
        } finally {
            try {
